...
#IF_DEFINED SNAPI_MONITOR_MODULE
    ...
    {
#END_IF
        // if this asset is ONLINE and has not already
        // been registered then register it with RMS now
        ...
        // if this asset is now ONLINE and has already
        // been registered with RMS, then we need to request
        // that the asset re-synchronize it's parameters
        // values since it is been offline for some period of time
        ELSE IF(DEVICE_ID(dvMonitoredDevice) && assetRegistered)
        {
          // skip this callback invocation if this
          // precompiler variable is detected
          #IF_NOT_DEFINED RMS_EXCLUDE_ASSET_SYNC_CALLBACK

            // perform synchronization of this asset's metadata
            SynchronizeAssetMetadataWrapper()

            // perform synchronization of this asset's parameters
            SynchronizeAssetParametersWrapper()
...
//
// (DUET VIRUTAL DEVICE DATA INITIALIZED EVENT HANDLER)
//
