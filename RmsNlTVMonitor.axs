 ...
 *     as applicable.
*/
MODULE_NAME='RmsNlTVMonitor'(DEV vdvRMS,
                             DEV vdvDeviceModule,
...
CHAR MONITOR_ASSET_NAME[]             = 'TV';


// RMS Asset Properties (Optional)
CHAR MONITOR_ASSET_DESCRIPTION[]      = 'LED TV';
CHAR MONITOR_ASSET_MANUFACTURERNAME[] = 'Samsung';
CHAR MONITOR_ASSET_MODELNAME[]        = 'ME46C';
...
#IF_DEFINED HAS_SOURCE_SELECT
    ...
    CHAR METADATA_PROPERTY_SOURCE_INPUT[100]  = 'HDMI 1|HDMI 2|Display Port|DVI 1|Computer';
#END_IF
...
#IF_DEFINED HAS_TUNER
    ...
    METADATA_PROPERTY_TUNER_BAND_COUNT     =  3;
    CHAR METADATA_PROPERTY_TUNER_BAND[100] = 'ATSC|NTSC|QAM';
#END_IF
...
DEFINE_FUNCTION RegisterAsset(RmsAsset asset)
{
  ...
  asset.assetType         = RMS_ASSET_TYPE_TV;
  ...
  IF(IsRmsReady())
  {
      IF ((lMinutes % 60) == 0)
      {
        IF(assetClientKey != '')
        {
          SWITCH(lTl)
          {
            CASE TL_MONITOR_POWER_ON : RmsAssetParameterUpdateValue(assetClientKey,'display.usage',RMS_ASSET_PARAM_UPDATE_OPERATION_INCREMENT,'1')
          }
        }
